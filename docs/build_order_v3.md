...
| Companions K1–K5 | companion_item_systems §10 | **K1 done and verified (2026-07-09, checklist 1–7 all passed** — `docs/k1_verification.md`): §4a formula live (20 + Σ⌊BaseHP/2⌋ + Devoted +2/Sworn +4), pool readout at launch, LoyaltyTier helper (Devoted/Sworn split 75/90 = working assumption, ruling still open), restore-clamp for mid-combat permadeath. **K2 done and verified (2026-07-09, all checklist items + D1–D4 passed** — `docs/k2_verification.md`): §5b rolls (tier 15/30/boss 40, Sworn −10) on lost combats + pool-zero wipes, InjuredLunationsRemaining serialized + round-trip asserted, three-demand exclusion (party/pool/envoys), infirmary recovery on the lunation tick (R24 interim), injury VISIBILITY (campus roster badge + deploy-dialog party manifest — post-verification UX addendum, U1–U3 pending a look). Shipped alongside: RULED defeat ends the expedition (dead party ≠ respawn); CheckCombatEnd re-entry guard (was emitting 4× per loss). Death ripple + signature destruction explicitly deferred to K4's loyalty hooks. **K2.5 RULED, built, and verified (2026-07-09 — E1–E8 all passed, k2_verification.md):** expedition HP persistence — unit HP carries between fights within an expedition (unit HP = the fights, pool = the journey; §4a single-currency position recorded and overruled), extraction infirmary gate (<25% BaseHP → 1 lunation; downed-in-won-fight → stabilized, out for the expedition, 1–2 lunations home), casualty notes on failure/extraction banners. "Sacrifice" (downing in a won fight) logged as a K4 loyalty-layer design item; companion_item_systems needs a v2.2 pass to absorb the amendment. K3–K5 not started: no hiring halls, no Muster screen. |
| Items Q1–Q5 | companion_item_systems §10 | **Q1 done; Q2 built (2026-07-10), in verification** (`docs/q2_verification.md`): item passives migrated onto the U3 trigger bus — ONE shared dispatcher (`BuildTriggeredEffect`) for enemy abilities AND items, `ItemDefinition.Trigger`/`ItemAbility`, three exemplars (Aegis Charm onSpawn / Duelist's Brand onAttack / Standard of the Vigil aura), bleed status tick, log grammar via FormatLogLine. Rulings: onAttack rides the stack, onSpawn resolves inline (no priority at spawn), aura is a state (§5). Legacy ItemPassiveTag enum path UNCHANGED for the 5 unmigrated tags. **Q3 built (2026-07-10), in verification** (`docs/q3_verification.md`): §4b traversal-resistance package — CorruptionWard (Σ across party, capped tier×2, floor 1), HazardWard (floor 1), Pathfinder (−step cost, floor 1, preview parity via the shared StepCost), party-sum accessors on EquipmentLoadout, CorruptionTierAt banding, three exemplars, `[PartyResist]` readout. §7b families whose hosts don't exist yet are DEFERRED with their deps (EssenceWell→S1 Essence pool, Chartwright→silhouette radius, Provisioner→rest-heal knob, court items→dispatch-takes-gear). Q4–Q5 not started (city markets, blighted items, relics, enchanting). |
