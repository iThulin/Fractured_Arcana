...
public static class WorldGenerator
{
    ...
    /// penalty. That is the same arithmetic StepCost charges the party, minus
    /// the spell adjustments (no expedition exists at worldgen). Water never
    /// entered.
    ...
    /// <see cref="StepCostMap"/>) lies in [minCost, maxCost]. The band is
    /// dimensioned against ExpeditionManager.OperatingRange, unlike the old
    /// crow-flies rings. Prefers road-adjacent sites (outposts sit on roads; a
    /// road approach floors near 1/step). Guarantees the exploration loop can
