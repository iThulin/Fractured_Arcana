...
public partial class HexGridManager : Node3D
{
    ...
    public HashSet<Vector2I> GetReachableTiles(Unit unit)
    {
        ...
        if (!unit.CanMove()) return result;  // no AP = no highlights
        ...
        while (frontier.Count > 0)
        {
            ...
            foreach (var neighbor in GetNeighbors(current))
            {
                ...
                if (neighbor != start) result.Add(neighbor);
            }
        }
    ...
    private void EnsureConnectivity(Vector2I start, Vector2I goal)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            if (current == goal) { connected = true; break; }

            foreach (var neighbor in GetNeighbors(current))
            {
                if (visited.Contains(neighbor)) continue;
                if (!Tiles.TryGetValue(neighbor, out var t)) continue;
                if (!t.IsWalkable || t.IsBlocked) continue;
                visited.Add(neighbor);
    ...
    public bool HasLineOfSight(Vector2I from, Vector2I to)
    {
        ...
        for (int i = 1; i < steps; i++)
        {
            ...
            if (!Tiles.TryGetValue(coord, out var tile)) continue;
    ...
    {
        if (unit?.CurrentTile == null) return null;
    ...
    public TileData GetFirstStepAwayFrom(Unit unit, Vector2I goal)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            foreach (var neighbor in GetNeighbors(current))
            {
                ...
                if (d > bestDist) { bestDist = d; bestDest = neighbor; }
            }
        }
