...
public static partial class CardScriptRegistry
{
    ...
    public static void RegisterBuiltins()
    {
        ...
        RegisterElementalistEffects();
        RegisterNecromancerEffects();
        RegisterArcanistEffects();
        RegisterEnchanterEffects();
        RegisterChronomancerEffects();
        RegisterTinkerEffects();
        RegisterDruidEffects();
        ...
        // Caster controls at least `min` constructs: { "type": "construct_count", "min": 2 }
        RegisterPredicate("construct_count", n =>
            new ConstructCountPredicate(n.TryGetProperty("min", out var mn) ? mn.GetInt32() : 1));

        // Primary target stands within 1 tile of a friendly construct: { "type": "target_adjacent_to_construct" }
        RegisterPredicate("target_adjacent_to_construct", _ => new TargetAdjacentToConstructPredicate());
        ...
        {
            int threshold = n.TryGetProperty("threshold", out var t) ? t.GetInt32() : 2;
            return new SpellsCastThisTurnPredicate(threshold);
        });
